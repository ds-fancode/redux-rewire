...
import {CreateActionSliceType} from './create-action-slice-legacy-type'
import {createActionsReference} from './create-actions-reference'

export const createActionSliceLegacy: CreateActionSliceType = function (
  reducerSlice,
  actionMap
) {
  return function (
    ...
    if (!actionsRef) {
      ...
      allAvailableActionKeys.forEach((actionKey) => {
        actions[actionKey] = (
          ...
          if (reducerActions[actionKey]) {
            reducerActions[actionKey]?.(data)
          }
